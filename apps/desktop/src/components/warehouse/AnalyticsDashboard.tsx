        ...
        <div className="p-4 space-y-6 overflow-y-auto h-full custom-scrollbar">
            <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">Historical token consumption and agent fleet metrics.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-xl p-4 relative overflow-hidden group shadow-sm">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-emerald-500">
                        <Zap size={48} />
                    </div>
                    <p className="text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">Total Input</p>
                    <h3 className="text-2xl font-black mb-1">{(stats.total_input / 1000).toFixed(1)}k</h3>
                    <p className="text-[10px] text-emerald-500/70 font-mono font-bold uppercase tracking-tight">tokens ingested</p>
                </div>
                ...
                <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-xl p-4 relative overflow-hidden group shadow-sm">
                    ...
                    <h3 className="text-2xl font-black mb-1">{(stats.total_output / 1000).toFixed(1)}k</h3>
                    <p className="text-[10px] text-primary/70 font-mono font-bold uppercase tracking-tight">tokens generated</p>
                </div>
                ...
                <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-xl p-4 relative overflow-hidden group shadow-sm">
                    ...
                    <h3 className="text-2xl font-black mb-1">{((stats.total_output / Math.max(stats.total_input, 1)) * 100).toFixed(1)}%</h3>
                    <p className="text-[10px] text-blue-500/70 font-mono font-bold uppercase tracking-tight">output ratio</p>
                </div>
            </div>

            {chartData.length > 0 && (
                <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-xl p-4 shadow-sm">
                    <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Token Burn Trajectory</h3>
            ...
            <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-sm">
                <div className="p-4 border-b border-white/5 flex items-center justify-between bg-muted/10">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white/5 text-[9px] uppercase tracking-widest font-black text-muted-foreground/60 border-b border-white/5">
                                ...
                                <tr key={session.id} className="hover:bg-white/5 transition-colors group">
                                    <td className="px-4 py-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Badge variant="outline" className="text-[9px] uppercase font-bold h-3.5 px-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                                {session.source || 'agent'}
                                            </Badge>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2">
                                        <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-colors">{session.project_name || 'Global'}</span>
                                    </td>
